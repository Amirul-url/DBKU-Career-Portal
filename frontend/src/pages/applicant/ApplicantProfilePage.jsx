      ...
      <ProfileFormRow label="Pengalaman Kerja">
        ...
        {form.hasExperience.startsWith("Ya") ? <div className="experience-details">
          ...
          {form.records.map((record, index) => <div className="experience-record" key={record.id}>
            ...
            <PersonalField label="DBKU"><input value={record.organisation} placeholder="Contoh. DBKU" onChange={(event) => setForm((current) => ({ ...current, records: current.records.map((item) => item.id === record.id ? { ...item, organisation: event.target.value } : item) }))} /></PersonalField>
            <PersonalField label="Negara" optional><PersonalSelect value={record.country} placeholder="Pilih negara" options={toSelectOptions(["Malaysia", "Brunei Darussalam", "Singapura", "Indonesia", "Thailand", "Filipina", "Vietnam", "China", "Jepun", "Korea Selatan", "Australia", "United Kingdom", "Amerika Syarikat"])} onChange={(event) => setForm((current) => ({ ...current, records: current.records.map((item) => item.id === record.id ? { ...item, country: event.target.value } : item) }))} /></PersonalField>
            ...
            <div className="experience-date-grid"><PersonalField label="Tarikh Mula - Bulan"><PersonalSelect value={record.startMonth} placeholder="Pilih" options={toSelectOptions(monthOptions)} onChange={(event) => setForm((current) => ({ ...current, records: current.records.map((item) => item.id === record.id ? { ...item, startMonth: event.target.value } : item) }))} /></PersonalField><PersonalField label="Tarikh Mula - Tahun"><PersonalSelect value={record.startYear} placeholder="Pilih" options={toSelectOptions(yearOptions)} onChange={(event) => setForm((current) => ({ ...current, records: current.records.map((item) => item.id === record.id ? { ...item, startYear: event.target.value } : item) }))} /></PersonalField><PersonalField label="Tarikh Akhir - Bulan"><PersonalSelect value={record.endMonth} placeholder="Pilih" options={toSelectOptions(monthOptions)} onChange={(event) => setForm((current) => ({ ...current, records: current.records.map((item) => item.id === record.id ? { ...item, endMonth: event.target.value } : item) }))} /></PersonalField><PersonalField label="Tarikh Akhir - Tahun"><PersonalSelect value={record.endYear} placeholder="Pilih" options={toSelectOptions(yearOptions)} onChange={(event) => setForm((current) => ({ ...current, records: current.records.map((item) => item.id === record.id ? { ...item, endYear: event.target.value } : item) }))} /></PersonalField></div>
