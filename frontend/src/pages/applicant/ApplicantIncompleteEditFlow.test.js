...
test("applicant can submit acceptance confirmation after organization feedback", () => {
  ...
  assert.match(viewSource, /payload\.append\("applicantConfirmationDocument", file\)/);
  assert.match(viewSource, /setSelectedFiles\(\(current\) => \[\.\.\.current, \.\.\.uploadedDocuments\]\)/);
  ...
  assert.match(viewSource, /document,\s*index/);
  assert.match(viewSource, /payload\.append\("applicantConfirmationDocuments", document\.file\)/);
  ...
  assert.match(viewSource, /disabled=\{hasResponded \|\| isSaving \|\| isUploadingPreview \|\| !selectedFiles\.length\}/);
...
test("internship back action asks applicant to save a visible draft", () => {
  ...
  assert.match(
    formSource,
    /Maklumat yang telah diisi akan disimpan sebagai draf dan dipaparkan dalam Permohonan Saya\./,
  );
  assert.match(formSource, /Simpan draf/);
  assert.doesNotMatch(formSource, /Keluar tanpa simpan/);
  assert.doesNotMatch(formSource, /student-save-draft-discard/);
  assert.doesNotMatch(formSource, /exitWithoutSavingDraft/);
...
test("internship mandatory tabs and fields show red required markers", () => {
  ...
  assert.match(formSource, /const activeInfoHeading = isJobApplication[\s\S]*getJobInfoHeading\(activeInfoTab, currentInfoTabs\.indexOf\(activeInfoTab\)\)[\s\S]*:\s*activeInfoTab/);
  assert.match(formSource, /<tr className="student-personal-section-heading">[\s\S]*<th colSpan=\{2\}>\{activeInfoHeading\}<\/th>/);
  ...
  assert.match(formSource, /\["supervisorEmail", "Emel Penyelaras Program"\]/);
  assert.match(formSource, /\["supervisorPhone", "No\. Telefon Penyelaras Program"\]/);
  assert.doesNotMatch(formSource, /renderPersonalRow\(\s*"Nama Penyelaras Program"[\s\S]*,\s*false,\s*\)/);
