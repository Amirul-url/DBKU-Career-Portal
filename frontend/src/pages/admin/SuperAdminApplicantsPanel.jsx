...
function JobPreferencesMirror({ data }) {
  ...
  const details = <div className="personal-edit-panel"><div className="personal-edit-form"><FormRow label="Keterlihatan Profil"><RadioGroup label="Adakah anda sedang mencari pekerjaan?" name="job-search-status" options={["Ya", "Tidak"]} value={preferences.isLookingForJob} /><p className="job-preference-note">Jika pemohon memilih 'Tidak', profil tidak akan berada dalam senarai padanan kekosongan oleh DBKU.</p></FormRow><FormRow label="Matlamat Kerjaya"><Field label="Matlamat kerjaya" value={preferences.careerObjective} multiline /></FormRow><FormRow label="Pilihan Pekerjaan">{jobs.length ? jobs.map((job, index) => <div className="job-preference-edit-card" key={job.id || index}><strong>Pilihan Pekerjaan {index + 1}</strong><Field label="Pilihan Pekerjaan" value={job.title} /><ReadOnlyCheckbox checked={job.hasRelatedExperience} label="Saya mempunyai pengalaman berkaitan kerja ini" /><SelectField label="Taraf Jawatan Pilihan" value={job.careerLevel} placeholder="Pilih tahap kerjaya" /><MultiField label="Sektor Pilihan" value={job.sectors} /><MultiField label="Kemahiran Berkaitan" value={job.skills} /><MultiField label="Status Pilihan Pekerjaan" value={job.employmentStatuses} /><MultiField label="Pilihan Waktu Bekerja" value={job.workTimes} /><MultiField label="Gaji Yang Dijangkakan (MYR)" value={job.expectedSalary} /><SelectField label="Negeri" value={job.state} optional placeholder="Pilih negeri" /><SelectField label="Bandar" value={job.city} optional placeholder="Pilih bandar" /><SelectField label="Jarak" value={job.distance} optional placeholder="Pilih jarak" /></div>) : <TextBlock>Tiada pilihan pekerjaan direkodkan.</TextBlock>}</FormRow></div></div>;
  return <ProfileMirrorCard title="Pilihan Pekerjaan" details={details}>{summary}</ProfileMirrorCard>;
}

function ExperienceMirror({ data }) {
  ...
  const details = <div className="personal-edit-panel experience-form"><div className="personal-edit-form"><FormRow label="Status Bekerja"><SelectField label="Status pekerjaan semasa" value={experience.employmentStatus} placeholder="Pilih status pekerjaan" /></FormRow><FormRow label="Pengalaman Kerja"><RadioGroup label="Adakah anda mempunyai pengalaman bekerja?" name="experience-status" options={["Ya, saya mula bekerja sejak:", "Tidak"]} value={experience.hasExperience} /><div className="personal-date-group"><span>Tarikh mula <em>(tidak wajib)</em></span><div><label>Bulan<input readOnly value={experience.startMonth || ""} /></label><label>Tahun<input readOnly value={experience.startYear || ""} /></label></div></div>{records.length ? records.map((record, index) => <div className="experience-record" key={record.id || index}><strong>Pengalaman {index + 1}</strong><Field label="Pekerjaan" value={record.title} /><SelectField label="Taraf Jawatan" value={record.careerLevel} placeholder="Pilih taraf jawatan" /><Field label="Syarikat" value={record.organisation} /><SelectField label="Negara" value={record.country} optional placeholder="Pilih negara" /><MultiField label="Sektor" value={record.sectors} /><div className="experience-date-grid"><Field label="Tarikh Mula - Bulan" value={record.startMonth} /><Field label="Tarikh Mula - Tahun" value={record.startYear} /><Field label="Tarikh Akhir - Bulan" value={record.isCurrent ? "" : record.endMonth} /><Field label="Tarikh Akhir - Tahun" value={record.isCurrent ? "" : record.endYear} /></div><ReadOnlyCheckbox checked={record.isCurrent} label="Saya masih bekerja di sini" /><Field label="Deskripsi Jawatan" value={record.description} optional multiline /><MultiField label="Kemahiran Berkaitan" value={record.skills} /><SelectField label="Purata Gaji (MYR)" value={record.salary} placeholder="Pilih purata gaji" /></div>) : <TextBlock>Tiada pengalaman kerja direkodkan.</TextBlock>}</FormRow></div></div>;
  return <ProfileMirrorCard title="Pengalaman" details={details}>{summary}</ProfileMirrorCard>;
}

function AcademicMirror({ data }) {
  ...
  const details = <div className="academic-layout"><strong className="academic-layout-label">Akademik</strong><div className="academic-form"><p className="academic-intro">Latar belakang akademik pemohon<span>*</span></p><div className="academic-record-list">{records.length ? records.map((record, index) => <section className="academic-record" key={record.id || index}><strong>Akademik {index + 1}</strong><SelectField label="Tahap Akademik" value={record.level} placeholder="Pilih tahap akademik" /><SelectField label="Bidang Akademik" value={record.fieldOfStudy} placeholder="Pilih bidang akademik" /><Field label="Pengkhususan" value={record.specialization} optional /><Field label="Nama Institusi Akademik" value={record.institution} /><SelectField label="Negara" value={record.country} placeholder="Pilih negara" /><Field label="Keputusan" value={record.result} optional />{record.spmGrades && hasValue(record.spmGrades) ? <Field label="Gred SPM" value={Object.entries(record.spmGrades).filter(([, value]) => hasValue(value)).map(([subject, grade]) => `${subject}: ${grade}`).join(", ")} optional /> : null}<div className="academic-date-section"><strong>Tarikh Mula <em>(tidak wajib)</em></strong><div className="academic-date-grid"><Field label="Bulan" value={record.startMonth} optional /><Field label="Tahun" value={record.startYear} optional /></div></div><div className="academic-date-section"><strong>Tarikh Akhir <em>(tidak wajib)</em></strong><div className="academic-date-grid"><Field label="Bulan" value={record.isStudying ? "" : record.endMonth} optional /><Field label="Tahun" value={record.isStudying ? "" : record.endYear} optional /></div></div><ReadOnlyCheckbox checked={record.isStudying} label="Saya sedang belajar di sini" /></section>) : <TextBlock>Tiada rekod akademik direkodkan.</TextBlock>}</div></div></div>;
  return <ProfileMirrorCard title="Akademik" details={details}>{summary}</ProfileMirrorCard>;
}

function SkillsMirror({ data }) {
  const skills = data || {};
  ...
  const details = <div className="skills-layout"><strong className="skills-layout-label">Set Kemahiran</strong><div className="skills-form"><strong>Set kemahiran yang dipilih<span>*</span></strong><p>Senarai kemahiran yang telah diisi oleh pemohon.</p><MultiField label="Kemahiran Berkaitan" value={skillList} /><MultiField label="Kemahiran MS Office" value={office} optional /><MultiField label="Lesen Memandu" value={licences} optional />{languages.length ? <div className="skills-language-section"><strong>Bahasa<span>*</span></strong>{languages.map((language, index) => <article key={language.id || index}><strong>Bahasa {index + 1}</strong><SelectField label="Bahasa" value={language.name} placeholder="Pilih bahasa" /><SelectField label="Tahap Pembacaan" value={language.reading} placeholder="Pilih tahap" /><SelectField label="Tahap Percakapan" value={language.speaking} placeholder="Pilih tahap" /><SelectField label="Tahap Penulisan" value={language.writing} placeholder="Pilih tahap" /></article>)}</div> : null}</div></div>;
  return <ProfileMirrorCard title="Kemahiran" details={details}>{summary}</ProfileMirrorCard>;
}
