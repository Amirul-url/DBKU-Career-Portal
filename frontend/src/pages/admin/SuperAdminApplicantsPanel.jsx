...
function SelectedList({ title, values }) {
  ...
  return <div className="job-selected-list"><strong>{title} ({selectedValues.length})</strong><div>{selectedValues.map((value) => <button type="button" key={value}><span>{value}</span><Icon>close</Icon></button>)}</div></div>;
}
...
function ExperienceMirror({ data }) {
  ...
  const details = <div className="personal-edit-panel experience-form"><div className="personal-edit-form"><FormRow label="Status Bekerja"><SelectField label="Status pekerjaan semasa" value={experience.employmentStatus} placeholder="Pilih status pekerjaan" /></FormRow><FormRow label="Pengalaman Kerja"><RadioGroup label="Adakah anda mempunyai pengalaman bekerja?" name="experience-status" options={["Ya, saya mula bekerja sejak:", "Tidak"]} value={experience.hasExperience} /><div className="experience-details"><strong>Tarikh mula <em>(tidak wajib)</em></strong><div className="personal-date-group"><Field label="Bulan" value={experience.startMonth} /><Field label="Tahun" value={experience.startYear} /></div><strong>Tambah pengalaman kerja anda*</strong><p>Tingkatkan peluang anda dengan memaparkan pengalaman kerja terdahulu.</p>{records.length ? records.map((record, index) => <div className="experience-record" key={record.id || index}><strong>Pengalaman {index + 1}</strong><Field label="Pekerjaan" value={record.title} /><SelectField label="Taraf Jawatan" value={record.careerLevel} placeholder="Pilih taraf jawatan" /><Field label="Syarikat" value={record.organisation} /><SelectField label="Negara" value={record.country} optional placeholder="Pilih negara" /><SelectField label="Sektor" value={asArray(record.sectors).length ? `${asArray(record.sectors).length} sektor dipilih` : ""} placeholder="Pilih satu atau lebih" /><SelectedList title="Sektor Ditambah" values={record.sectors} /><div className="experience-date-grid"><Field label="Tarikh Mula - Bulan" value={record.startMonth} /><Field label="Tarikh Mula - Tahun" value={record.startYear} /><Field label="Tarikh Akhir - Bulan" value={record.isCurrent ? "" : record.endMonth} /><Field label="Tarikh Akhir - Tahun" value={record.isCurrent ? "" : record.endYear} /></div><ReadOnlyCheckbox checked={record.isCurrent} label="Saya masih bekerja di sini" /><Field label="Deskripsi Jawatan" value={record.description} optional multiline /><JobSearchPreview label="Kemahiran Berkaitan" value="" placeholder="Contoh. Perform market research" helper="Anda boleh membuat penambahan kemahiran yang anda miliki secara manual." /><SelectedList title="Kemahiran Anda" values={record.skills} /><SelectField label="Purata Gaji (MYR)" value={record.salary} placeholder="Pilih purata gaji" /></div>) : <TextBlock>Tiada pengalaman kerja direkodkan.</TextBlock>}</div></FormRow></div></div>;
