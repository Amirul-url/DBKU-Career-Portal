...
import {
  AcademicSummary,
  ApplicantPersonalReadOnlyView,
  ApplicantProfileReadOnlyCards,
  ExperienceSummary,
  SkillsSummary,
} from "../applicant/ApplicantProfilePage";
...
function SharedSummaryMirror({ children, isOpen, onToggle, title }) {
  const body = <div className="px-7 py-6">{children}</div>;
  return <ProfileMirrorCard title={title} details={body} isOpen={isOpen} onToggle={onToggle}>{body}</ProfileMirrorCard>;
}

function ApplicantExperienceMirror({ data, isOpen, onToggle }) {
  return <SharedSummaryMirror title="Pengalaman" isOpen={isOpen} onToggle={onToggle}><ExperienceSummary data={{ ...data, records: asArray(data?.records) }} /></SharedSummaryMirror>;
}

function ApplicantAcademicMirror({ data, isOpen, onToggle }) {
  return <SharedSummaryMirror title="Akademik" isOpen={isOpen} onToggle={onToggle}><AcademicSummary data={{ ...data, records: asArray(data?.records) }} /></SharedSummaryMirror>;
}

function ApplicantSkillsMirror({ data, isOpen, onToggle }) {
  return <SharedSummaryMirror title="Kemahiran" isOpen={isOpen} onToggle={onToggle}><SkillsSummary data={data || {}} /></SharedSummaryMirror>;
}
...
export default function SuperAdminApplicantsPanel({ allowDelete = true, compact = false }) {
  ...
  return <section className={compact ? "p-0" : "p-7"}><div className="mb-6"><h1 className="text-2xl font-bold text-slate-950">Pengurusan Pemohon</h1><p className="mt-1 text-sm text-slate-500">Semak akaun pemohon dan maklumat profil mereka.</p></div><form className="mb-5 flex gap-2 rounded-lg border border-slate-200 bg-white p-2.5" onSubmit={(event) => { event.preventDefault(); loadApplicants(); }}><input className="min-w-0 flex-1 rounded-md border border-slate-300 px-3 py-1.5 text-xs" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Cari nama, MyKad, emel atau nombor telefon" /><button className="superadmin-compact-control inline-flex items-center gap-1.5 rounded-md bg-emerald-700 px-3 py-1.5 text-xs font-bold text-white hover:bg-emerald-800" type="submit"><Icon>search</Icon>Tapis</button><button className="rounded-md border border-slate-300 px-3 text-xs font-bold text-slate-600" type="button" onClick={() => { setQuery(""); loadApplicants(""); }}>Set Semula</button></form><div className="overflow-hidden rounded-lg border border-slate-200 bg-white"><header className="border-b border-slate-200 px-5 py-3.5"><h2 className="text-base font-bold text-slate-950">Senarai Pemohon</h2><p className="mt-1 text-sm text-slate-500">{applicants.length} pemohon dijumpai.</p></header>{error ? <p className="m-5 rounded-md bg-red-50 p-4 text-sm font-semibold text-red-700">{error}</p> : null}<div className="overflow-x-auto"><table className="min-w-full text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="w-20 px-5 py-3.5">No.</th><th className="px-5 py-3.5">Nama</th><th className="px-5 py-3.5">Nombor MyKad</th><th className="px-5 py-3.5">Emel</th><th className="px-5 py-3.5">Nombor Telefon</th><th className="px-5 py-3.5">Tindakan</th></tr></thead><tbody>{loading ? <tr><td className="px-5 py-6 text-slate-500" colSpan="6">Memuatkan pemohon...</td></tr> : visibleApplicants.length ? visibleApplicants.map((applicant, index) => <tr className="border-t border-slate-100" key={applicant.id}><td className="px-5 py-3.5 font-semibold text-slate-500">{pageStart + index + 1}</td><td className="px-5 py-3.5 text-sm font-bold text-slate-900">{applicant.full_name || applicant.first_name}</td><td className="px-5 py-3.5 text-sm text-slate-600">{text(applicant.mykad_number)}</td><td className="px-5 py-3.5 text-sm text-slate-600">{applicant.email}</td><td className="px-5 py-3.5 text-sm text-slate-600">{text(applicant.mobile_number)}</td><td className="px-5 py-3.5"><div className="flex gap-2"><button className="app-view-action" type="button" onClick={() => viewProfile(applicant.id)}><Icon>visibility</Icon>{loadingProfile ? "Memuatkan" : "Lihat"}</button>{allowDelete ? <button className="inline-flex items-center gap-2 rounded-md border border-red-200 px-3 py-2 text-sm font-bold text-red-600 hover:bg-red-50" type="button" onClick={() => setDeleteTarget(applicant)}><Icon>delete</Icon>Padam</button> : null}</div></td></tr>) : <tr><td className="px-5 py-6 text-slate-500" colSpan="6">Tiada pemohon ditemui.</td></tr>}</tbody></table></div><footer className="flex items-center justify-between border-t border-slate-200 px-5 py-4 text-sm text-slate-500"><span>{applicants.length ? `Memaparkan ${pageStart + 1}-${Math.min(pageStart + APPLICANTS_PER_PAGE, applicants.length)} daripada ${applicants.length}` : "Tiada rekod untuk dipaparkan."}</span><div className="flex items-center gap-2"><button className="inline-flex h-9 w-9 items-center justify-center rounded-md border border-slate-300 font-bold text-slate-700 disabled:cursor-not-allowed disabled:opacity-40" type="button" onClick={() => setCurrentPage((page) => Math.max(1, page - 1))} disabled={activePage === 1} aria-label="Halaman sebelumnya">&lt;</button><span className="font-semibold text-slate-700">{activePage} / {pageCount}</span><button className="inline-flex h-9 w-9 items-center justify-center rounded-md border border-slate-300 font-bold text-slate-700 disabled:cursor-not-allowed disabled:opacity-40" type="button" onClick={() => setCurrentPage((page) => Math.min(pageCount, page + 1))} disabled={activePage === pageCount} aria-label="Halaman seterusnya">&gt;</button></div></footer></div><ApplicantProfileModal data={profile} onClose={() => setProfile(null)} />{allowDelete ? <ApplicantDeleteModal applicant={deleteTarget} deleting={deleting} onCancel={() => setDeleteTarget(null)} onConfirm={confirmDeleteApplicant} /> : null}</section>;
}
