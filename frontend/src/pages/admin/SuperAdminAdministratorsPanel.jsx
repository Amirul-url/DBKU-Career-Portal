  ...
  return (
    <section className="p-7">
      <div className="mb-6 flex items-start justify-between gap-4">
        ...
        <button className="superadmin-compact-control inline-flex items-center gap-1.5 rounded-md bg-emerald-700 px-3 py-1.5 text-xs font-bold text-white hover:bg-emerald-800" type="button" onClick={openAddModal}>
          <Icon>person_add</Icon>
          Tambah Akaun
        </button>
      </div>

      <form className={`mb-5 grid gap-2 rounded-lg border border-slate-200 bg-white p-2.5 ${config.hasDepartment ? "lg:grid-cols-[minmax(0,1fr)_260px_auto_auto]" : "lg:grid-cols-[minmax(0,1fr)_auto_auto]"}`} onSubmit={(event) => { event.preventDefault(); loadAccounts(); }}>
        <input className="min-w-0 rounded-md border border-slate-300 px-3 py-1.5 text-xs" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={config.searchPlaceholder} />
        {config.hasDepartment ? (
          <select className="rounded-md border border-slate-300 px-3 py-1.5 text-xs" value={department} onChange={(event) => setDepartment(event.target.value)}>
            <option value="">Semua jabatan</option>
            {departmentOptions.map((option) => <option value={option} key={option}>{option}</option>)}
          </select>
        ) : null}
        <button className="superadmin-compact-control inline-flex items-center justify-center gap-1.5 rounded-md bg-emerald-700 px-3 py-1.5 text-xs font-bold text-white hover:bg-emerald-800" type="submit">
          <Icon>search</Icon>
          Tapis
        </button>
        <button className="rounded-md border border-slate-300 px-3 text-xs font-bold text-slate-600" type="button" onClick={() => { setQuery(""); setDepartment(""); loadAccounts("", ""); }}>Set Semula</button>
      </form>
